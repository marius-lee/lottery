...
def run():
    ...
    has_bias = (
        len(boot_sig_bonf) > 0 or       # Bonferroni显著
        eb["overdispersion"] > 1e-6 or  # 有过离散
        len(consensus_up) > 0 or         # 位置一致性
        l3["chi2_significant"]           # 蓝球χ²显著
    )

    if has_bias:
        print(f"⚠️  检测到统计显著的频率偏差")
        print(f"  → 彩票不是均匀分布的")
        print(f"  → 静态偏差可被利用")

        if eb["overdispersion"] > 1e-6:
            print(f"  → 经验贝叶斯过离散={eb['overdispersion']:.6f} > 0")
            print(f"    确认频率方差异于二项分布期望")
        if len(consensus_up) > 0:
            print(f"  → {len(consensus_up)}个号码在所有6个位置都偏高")
            print(f"    跨位置一致性暗示物理偏差 (非统计噪声)")
        if len(boot_sig_bonf) > 0:
            print(f"  → {len(boot_sig_bonf)}个号码Bonferroni校正后仍显著")
            print(f"    最极端偏差: {', '.join(f'#{n}' for n,_ in boot_sig_bonf[:5])}")

        print()
        print(f"  建议策略:")
        print(f"    1. 选号优先: {l4['top6_bias_numbers']}")
        print(f"    2. 回避: {l4['bottom6_avoid_numbers']}")
        print(f"    3. 组合增益 ~{l4['top6_probability_boost_pct']:+.1f}% (相对随机)")
        print(f"    4. 结合覆盖设计: 在偏差号码集上做t-wise覆盖")
    else:
        print(f"✅ 无显著频率偏差")
        print(f"  → 彩票在静态频率上接近均匀分布")
        print(f"  → 偏差存在但太小 (<1%), 需要更多数据或更精细模型")
