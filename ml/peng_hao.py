...
def _get_values(data, position):
    """从data中提取指定位置的数值序列."""
    if position < 6:
        return [row[position + 1] for row in data]
...
def compute_channel(data, position=0, period=None):
    ...
    values = _get_values(data, position)
...
def compute_all_channels(data):
    """计算全部7个位置的通道."""
    return {f"pos_{pos}": compute_channel(data, pos) for pos in range(7)}
...
def _weighted_markov_dir(data, position=0):
    ...
    vals = _get_values(data, position)
    if len(vals) < 4:
        return None
    ...
    # 步长3 (仅当数据充足时)
    dir3 = classify_3_direction([vals[-4], vals[-1]]) if len(vals) >= 5 else dir1
...
def generate_tickets(data, n=3, use_channel=True, use_direction=True, use_extreme=True):
    ...
    # 3. 极端值规则叠加 [彭浩 Ch4]
    extreme_pred = {}
    extreme_alerts_result = extreme_rules(data)
    if use_extreme:
        for alert in extreme_alerts_result.get("alerts", []):
    ...
    return {
        ...
        "extreme_alerts": extreme_alerts_result.get("alerts", []),
