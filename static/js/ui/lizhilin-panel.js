  ...
  function render(d){
    ...
    if(d && d.ok){
      if(d.dan_pool && d.dan_pool.length){
        h += '<details style="margin-bottom:6px;"><summary style="color:#A78BFA;font-size:14px;cursor:pointer;">🎯 定胆候选: '+d.dan_pool.length+'个</summary>';
        h += '<div style="font-size:16px;color:#A78BFA;margin-top:4px;">'+d.dan_pool.map(function(n){return String(n).padStart(2,'0')}).join(' ')+'</div></details>';
      }
      if(d.trans_pool && d.trans_pool.length){
        h += '<details style="margin-bottom:6px;"><summary style="color:#F59E0B;font-size:14px;cursor:pointer;">🔗 带出表候选: '+d.trans_pool.length+'个</summary>';
        h += '<div style="font-size:16px;color:#F59E0B;margin-top:4px;">'+d.trans_pool.map(function(n){return String(n).padStart(2,'0')}).join(' ')+'</div></details>';
      }
      if(d.kill_excluded && d.kill_excluded.length){
        h += '<details style="margin-bottom:6px;"><summary style="color:#EF4444;font-size:14px;cursor:pointer;">🗑 27杀号排除: '+d.kill_excluded.length+'个</summary>';
        h += '<div style="font-size:16px;color:#EF4444;margin-top:4px;">'+d.kill_excluded.map(function(n){return String(n).padStart(2,'0')}).join(' ')+'</div></details>';
      }
      if(d.red_pool){
        h += '<div style="margin-bottom:8px;font-size:16px;color:#10B981;">📐 红球候选池: '+d.red_pool_size+'个</div>';
      }
      ...
      if(d.blue_methods){
        ...
        if(bms.tail12 && bms.tail12.survived !== undefined){
          h += '<details style="margin-bottom:6px;"><summary style="color:#3B82F6;font-size:14px;cursor:pointer;">🔵 12种尾数杀号: '+bms.tail12.survived.length+'蓝</summary>';
          h += '<div style="font-size:16px;color:#3B82F6;margin-top:4px;">'+bms.tail12.survived.map(function(n){return String(n).padStart(2,'0')}).join(' ')+'</div></details>';
        }
        if(bms.ten_kill && bms.ten_kill.survived !== undefined){
          h += '<details style="margin-bottom:6px;"><summary style="color:#06B6D4;font-size:14px;cursor:pointer;">🟦 十招杀蓝: '+bms.ten_kill.survived.length+'蓝</summary>';
          h += '<div style="font-size:16px;color:#06B6D4;margin-top:4px;">'+bms.ten_kill.survived.map(function(n){return String(n).padStart(2,'0')}).join(' ')+'</div></details>';
        }
        if(bms.period && bms.period.survived !== undefined){
          h += '<details style="margin-bottom:6px;"><summary style="color:#0EA5E9;font-size:14px;cursor:pointer;">🔹 期号排除: '+bms.period.survived.length+'蓝</summary>';
          h += '<div style="font-size:16px;color:#0EA5E9;margin-top:4px;">'+bms.period.survived.map(function(n){return String(n).padStart(2,'0')}).join(' ')+'</div></details>';
        }
      }
    }
