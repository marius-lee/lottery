...
window._showPengPanel = function(){
  ...
  function render(ch, di){
    ...
    if(ch && ch.ok && ch.positions){
      ...
      h += '<div style="margin-bottom:10px;padding:6px 10px;background:rgba(239,68,68,0.08);border-radius:6px;font-size:11px;">';
    ...
    // ═══ 通道显示区 ═══
    h += '<h5 style="color:#3B82F6;font-size:13px;margin:8px 0;">📊 五均线号码通道 [彭浩 Ch3 §2]</h5>';
    h += '<div style="font-size:10px;color:#64748B;margin-bottom:6px;">18期MA + 3σ/√σ · 通道准确率=±1范围内概率 · 绿色=正常 黄色=警戒 红色=越界</div>';
    ...
    if(ch && ch.ok && ch.positions){
      for(var i=0; i<7; i++){
        ...
        h += '<div style="margin:4px 0;padding:6px 8px;border-radius:4px;background:'+bg+';font-size:11px;">';
      ...
      h += '<div style="color:#EF4444;font-size:11px;">'+(ch&&ch.msg||'加载失败')+'</div>';
    }

    // ═══ 方向预测区 ═══
    h += '<h5 style="color:#A78BFA;font-size:13px;margin:12px 0 8px 0;">🎯 方向预测 [彭浩 Ch3 §4]</h5>';
    h += '<div style="font-size:10px;color:#64748B;margin-bottom:6px;">三方向: ↓下=本期<上期  →平=本期=上期  ↑上=本期>上期 | 九方向: 三期组合 | 反转形态(~30%): 下上/上下</div>';
    ...
    if(di && di.ok && di.positions){
      ...
      for(var i=0; i<7; i++){
        ...
        var rev = d9==='下上' || d9==='上下';
        h += '<div style="padding:4px 8px;border-radius:4px;background:rgba(168,85,247,0.06);font-size:11px;text-align:center;min-width:60px;">';
        ...
        h += '<div style="font-size:9px;color:#64748B;">'+d9+(rev?' ⚡':'')+'</div>';
        h += '</div>';
      }
      h += '</div>';
      // 反转告警
      ...
      for(var i=0; i<7; i++){
        ...
        if(dd && (dd.current_9_dir==='下上' || dd.current_9_dir==='上下')){
          if(!hasReversal){ h += '<div style="margin-top:6px;font-size:10px;">'; hasReversal=true; }
          h += '<span style="color:#FBBF24;">⚡ 反转形态: '+['红一','红二','红三','红四','红五','红六','蓝球'][i]+'('+dd.current_9_dir+') </span>';
        }
      }
      if(hasReversal) h += '</div>';
    } else {
      h += '<div style="color:#EF4444;font-size:11px;">'+(di&&di.msg||'加载失败')+'</div>';
    }

    // ═══ 出号区 ═══
    h += '<div style="margin-top:14px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">';
    h += '<span style="font-size:12px;color:#94A3B8;font-weight:600;">出号选项:</span>';
    ...
    h += '<button class="btn btn-draw" onclick="window._pengDraw()" style="font-size:12px;padding:6px 16px;margin-left:auto;">彭浩出号</button>';
    h += '</div>';
...
window._pengDraw = async function(){
  ...
  var h = '<div style="font-size:10px;color:'+PU.GRAY+';margin-bottom:6px;">'+
